...
function sap() {
  if [ -z "$1" ] 
  then
    export ACTIVE_PROJECT="$(echo $PWD)"
  else
    export ACTIVE_PROJECT=$1
  fi
}

# Echo back the current active project
alias cap='echo $ACTIVE_PROJECT'
# Move to current active project
alias gta='cd $ACTIVE_PROJECT'
